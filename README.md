...
Findings land in-thread; containment approvals are buttons. A mention inside an
existing incident thread is treated as a follow-up question, answered from that
incident's record. Every request is signature-verified with a 5-minute replay
window, and handlers ack inside Slack's 3-second budget.
